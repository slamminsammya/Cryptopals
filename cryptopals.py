...
def score(text):
  ...
  if total != 0:
    ...
    #letter_frequency = {
    #  x : float(text.count(x)) / total for x in frequency.keys()
    #}
  else: return 100000
...
def getkey_xor(ciphertext):
  ...
  for k in chars:
    text = repeated_xor(k, ciphertext).decode('hex')
    scores[k] = score(text)
  answer = min(scores, key=scores.get)
  sort = sorted(scores.items(), key = lambda s: s[1])
  #for x in sort[:3]:
  #  try:
  #    print x[0], repeated_xor(x[0],ciphertext).decode('hex')
  #  except UnicodeDecodeError:
  #    print "cant print that"
  return sort[:7]
  #return answer
...
def find_xor(strings):
  #in a list of strings where only one has been encrypted via a fixed xor
  possibilities = []
  for x in strings:
    best = getkey_xor(x)
    best.append(strings.index(x))
...
for n in range(1,5):
  ...
  for i in range(2,41):
    edit_distance = av_distance(cipher, n, i)  
    key_length_scores[i] = edit_distance
  key_length_scores = sorted(key_length_scores.items(), key=lambda s: s[1])
  possible_key_lengths = [key_length_scores[i][0] for i in range(4)]
  print possible_key_lengths

decrypt = {
  
}
possible_lengths = [5,3,2,7, 29]

"""for x in possible_lengths:  
  blocks = ith_characters(cipher, x)
  key = ''
  for y in blocks:
    k_i = getkey_xor(y)
    print k_i
    #for z in k_i:
      #print repeated_xor(z[0],y).decode('hex')
    key += k_i[0][0]
    #key += k_i
  print repeated_xor(key,cipher).decode('hex)'), 'this was with keylength', x"""
...
ciphertext = """CRIwqt4+szDbqkNY+I0qbDe3LQz0wiw0SuxBQtAM5TDdMbjCMD/venUDW9BL
...
print aes128_decrypt(ciphertext,key)
  
def repeat_score(text):
  blocks = parse_as_blocks(text, 16)
  repeats = {
  }
  for x in blocks:
    if x in repeats.keys():
      repeats[x] += 1
    else:
      repeats[x] = 0 
  repeat_count = 0
  print repeats
  for key in repeats:
    repeat_count += repeats[key]
  return repeat_count
  
texts = """
...
for i in range(len(texts)):
  texts[i] = texts[i].decode('hex')
  best = max(repeat_score(texts[i]), best)
